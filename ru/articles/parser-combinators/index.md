...
сказал человек](https://en.wikipedia.org/wiki/Shunry%C5%AB_Suzuki#Quotations), много возможностей. По мнению эксперта, есть только один правильный вариант.
...
но вводить его лучше, так как это автоматически гарантирует вам
что вы его прочитайте полностью). Все части кода, которые Вам понадобятся, расположены в статье по порядку.  Имейте в виду, что иногда вводяться *изменённые*
...
открывающему тегу, и закрывающий символ `>`.
...
кавычки, и они заканчиваются на следующей, и все.
...
Что ещё хуже, `char` не обязательно даже то, что вы думаете о нем как о символе Unicode. Это, скорее всего, будет то, что Unicode называет ["grapheme
...
Мы сопоставляем с шаблоном `Some('a')`, что является конкретным результатом, который мы ищем,
...
Если мы получим любой другой `Some(char)`, или если мы получим `None`, то возвращаем ошибку. Как
...
В третьем, мы вводим неверный ввод: `"Hello Mike!"` и обратите внимание, что он действительно отклоняет ввод с ошибкой. Не то, чтобы `Майк` не правильно, как правило просто не тот, что искал парсер.
...
Вспомним правило для идентификатора имени элемента, оно выглядит следующим образом: один
...
На втором шаге мы продолжаем вытаскивать символы из итератора, `is_alphanumeric()` `String`, пока не найдём тот, который не является `is_alphanumeric()` (это похоже на `is_alphabetic()` за исключением того, что он также соответствует числам в любом алфавите) или тире `'-'`.

В первый раз, когда мы видим что-то, что не соответствует этим критериям, это означает, что мы закончили синтаксический анализ, поэтому мы прерываем цикл и возвращаем `String`, не забывая вырезать фрагмент, который мы использовали из `input`. Аналогично, если в итераторе заканчиваются символы, это означает, что мы достигли конца ввода.
...
Этот комбинатор имеет одну цель: изменить тип результата. Например, допустим, у вас есть парсер, который возвращает `((), String)` и вы хотели изменить его, чтобы он например возвращал только эту `String`.
...
Здесь следует особо отметить одну вещь: вместо непосредственного вызова синтаксического анализатора как функции, мы теперь должны перейти к `parser.parse(input)`, потому что мы не знаем, является ли тип `P` функцией, мы просто знаем, что он реализует `Parser`, и поэтому мы должны придерживаться интерфейса, который обеспечивает `Parser`. В противном случае тело функции выглядит точно так же, а типы выглядят намного аккуратнее. Появился новый параметр времени жизни `'a'`, ка источник дополнительного шума, но в целом это довольно значительное улучшение.
...
В дополнение к изменению типа возвращаемого значения, мы также должны убедиться, что тип входных данных для замыкания - `&'a str`, или `rustc` расстроится.
...
Глядя на этот код, насколько легко было бы адаптировать его к идее *ноль* или более? Нам просто нужно удалить первый запуск парсера:
...
На самом деле, подожди минутку. Мы не хотим анализировать пробелы, а *затем* анализировать атрибуты. Если вы думаете об этом, если нет атрибутов, пробел является необязательным, и мы можем столкнуться с немедленным `>` или `/>`. Но если есть атрибут, сначала *должен* быть пробел. К счастью для нас, между каждым атрибутом также должен быть пробел, если их несколько, поэтому мы действительно рассматриваем здесь последовательность из *нуля или более* вхождений *одного или нескольких* пробелов, за которыми следует атрибут.
...
Вложенность комбинаторов становится немного раздражающей в этот момент, но мы будем сопротивляться рефакторингу всего, чтобы исправить это только сейчас, и вместо этого сосредоточимся на том, что здесь происходит.
...
Мы воспользуемся нашей новой `pred` и `any_char`, чтобы получить парсер, который не принимает *ничего кроме другой кавычки*, что в `zero_or_more`. То о чём мы говорим, заключается в следующем:

- одна кавычка
- следуют ноль или более символов, которые *не* являются другой кавычкой
...
Прежде чем продолжить, вам лучше закомментировать эти две функции и тесты, пока мы это не исправим исправим ...
...
Очень много `'a`, но, увы все они необходимы. К счастью, мы все ещё можем повторно использовать наши старые функции комбинаторы без изменений - и в качестве дополнительного бонуса, мы не только получаем более приятный синтаксис для их применения, мы также избавляемся от взрывоопасных типов `impl Trait`.
...
И стоило поместить  `map` и `pred` в `Box` - *и* мы получили более приятный синтаксис!
...
`and_then` - это ещё одна вещь, которую вы видите в Rust, в основном в тех же местах, что и `map`. Он называется [`flat_map`](https://doc.rust-lang.org/std/iter/trait.Iterator.html#method.flat_map) в `Iterator`, но это тот же шаблон, что и остальные.
...
Это также можно назвать ленивым. Например, если у вас есть `Future<A>` которая все ещё ожидает разрешения, вместо `and_then` немедленно вызывающего функцию для создания `Future<B>`, вместо этого он создаёт новую `Future<B>` которая содержит обе `Future<A>` и функцию, которая затем ожидает завершения `Future<A>`. Когда это происходит вызывается функция с результатом `Future<A>`, и Боб - твой дядя <sup><a href="#footnote1">1</a></sup>, ты возвращаешь `Future<B>`. Другими словами, в случае `Future` вы можете думать о функции, которую вы передаёте `and_then` как о *функции обратного вызова*, потому что она с результатом исходной `Future`, когда она завершается. Это также немного более интересно, чем это, потому что она возвращает *новую* `Future`, которая может или не может уже завершиться, так что путь к *цепочке* `Future`s вместе.

Как и в случае с функторами, система типов Rust в настоящее время не способна выражать монады, поэтому отметим только, что этот шаблон называется монадой, и что, к сожалению, он не имеет ничего общего с буррито, в отличие от того, что скажут в интернете, двигайся дальше.
