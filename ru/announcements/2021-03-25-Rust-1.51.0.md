...
Данное изменение теперь позволяет иметь собственную структуру массива, обобщенную по типу *и* длине. Давайте посмотрим на пример определения и то, как его можно использовать.
...
Это была давняя проблема в Cargo, и с этим выпуском есть новая опция `resolver` в вашем `Cargo.toml`, где вы можете установить `resolver="2"`, чтобы попробовать новый подход к разрешению features. Вы можете ознакомиться с [RFC 2957](https://rust-lang.github.io/rfcs/2957-cargo-features2.html) для получения подробного описания поведения, которое можно резюмировать следующим образом.
...
- **Target dependencies** — Если у зависимости есть платформо-зависимые features, и она встречается в графе сборки несколько раз, то для неё включены будут только features текущей платформы сборки.
