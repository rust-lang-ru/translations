...
Теперь у Cargo есть [подкоманда `info`](https://doc.rust-lang.org/nightly/cargo/commands/cargo-info.html) для вывода информации о пакете в реестре. Мы выполнили [давнюю просьбу](https://github.com/rust-lang/cargo/issues/948), которой не исполнилось и 10 лет! За прошедшие годы было написано несколько сторонних расширений, подобных этому, и эта реализация была разработана в рамках [cargo-information](https://crates.io/crates/cargo-information) перед добавлением в Cargo.

Как пример, здесь вы можете увидеть вывод `cargo info cc`:
...
#### macOS на 64битном ARM теперь на 1 уровне

Платформа `aarch64-apple-darwin` для macOS на 64битном ARM (семейство чипов M1 и более молодые Apple Silicon CPU) теперь на 1 уровне поддержки, что означает высочайшие гарантии правильной работы. Как описано на [странице поддержки платформ](https://doc.rust-lang.org/stable/rustc/platform-support.html), каждое изменение в репозитории Rust должно пройти все тесты на каждой платформе первого уровня прежде чем оно может быть принято. Эта платформа была добавлена на 2 уровень поддержки в Rust 1.49, благодаря чему она появилась в `rustup`, а текущее изменение ставит её в один ряд с Linux на ARM64 и macOS, Linux или Windows на x86.

#### Платформа Mac Catalyst теперь на 2 уровне

[Mac Catalyst](https://developer.apple.com/mac-catalyst/) - это технология Apple, которая позволяет запускать iOS приложения нативно на Mac. Это очень удобно при тестировании iOS-специфичного кода. По большей части за счёт этого `cargo test --target=aarch64-apple-ios-macabi --target=x86_64-apple-ios-macabi` просто работает (в контрасте с обычными целевыми платформами iOS, которые должны быть в опакечены сторонними инструментами до того, как они смогут быть запущены на устройстве или в симуляторе).

[Эта целевая платформа](https://doc.rust-lang.org/nightly/rustc/platform-support/apple-ios-macabi.html) теперь на 2 уровне и может быть загружена при помощи `rustup target add aarch64-apple-ios-macabi x86_64-apple-ios-macabi`, так что сейчас самое время обновить ваши пайплайны чтобы тестировать ваш кот также и в iOS-подобном окружении.

### Точный захват в синтаксисе `use<..>`

Rust теперь поддерживает синтаксис `use <..>` в пределах определенных границ `impl Trait`, для управления тем, какие общие параметры времени жизни будут захвачены.

Типы в возвращаемом значении `impl Trait` (RPIT) в Rust *захватывают* определённые общие параметры. Захват этих общих параметров позволяет использовать их в скрытых типах, что влияет на анализатор заимствований.

В Rust 2021 и более ранних редакциях параметры времени жизни не отображаются в непрозрачных типах для простых функций, а также для функций и методов с встроенными значениями, если только эти параметры времени жизни не указаны синтаксически в непрозрачном типе.  Например, это ошибка:
...
С новым синтаксисом `use<..>` мы можем исправить это, как написано в ошибке, написав следующим образом:

```rust
fn f(x: &()) -> impl Sized + use<'_> { x }
```

Ранее для корректного исправления этого класса ошибок требовалось определить фиктивный трейт, условно называемый `Captures`, и использовать его следующим образом:
...
Это называлось ["трюк с `Captures`"](https://github.com/rust-lang/rfcs/blob/master/text/3498-lifetime-capture-rules-2024.md#the-captures-trick). В этом больше нет необходимости.

Существовал менее корректный, но более удобный способ исправить ошибку, который часто использовался под названием [ "трюк с выживанием"](https://github.com/rust-lang/rfcs/blob/master/text/3498-lifetime-capture-rules-2024.md#the-outlives-trick).  Ранее компилятор даже предлагал это сделать.  Этот трюк выглядел следующим образом:

```rust
fn f(x: &()) -> impl Sized + '_ { x }
```

В этом простом случае трюк в точности эквивалентен `+ use<'_>` по причинам, описанным в [RFC 3498](https://github.com/rust-lang/rfcs/blob/master/text/3498-lifetime-capture-rules-2024.md). Однако в реальных случаях это приводит к чрезмерному ограничению возвращаемого непрозрачного типа, что приводит к проблемам.  Например, рассмотрим этот код, который основан на реальном примере компилятора Rust:
...
Мы не можем удалить `+ 'cx`, поскольку время жизни используется в скрытом типе и поэтому должно быть зафиксировано.  Мы также не можем добавить ограничение для `'a: 'cx`, поскольку эти периоды жизни на самом деле не связаны, и в целом неверно, что `'a` переживает `'cx`.  Однако, если мы напишем вместо этого `+ use<'cx, 'a>`, это сработает и будет иметь правильные границы.

Есть некоторые ограничения в том, что мы стабилизируем сегодня.  Синтаксис `use<..>` в настоящее время не может отображаться в трейтах или в имплементации трейтов (но обратите внимание, что там параметры времени жизни в области видимости уже записаны по умолчанию), и в нем должны быть перечислены все параметры общего типа и const в области видимости.  Мы надеемся со временем снять эти ограничения.

Обратите внимание, что в Rust 2024 приведенные выше примеры будут "просто работать" без необходимости использования синтаксиса `use<..>` (или каких-либо хитростей).  Это связано с тем, что в новой редакции непрозрачные типы будут автоматически фиксировать все параметры времени жизни в области видимости.  Это лучший вариант по умолчанию, и мы видели много свидетельств того, как это очищает код.  В Rust 2024 синтаксис `use<..>`, будет служить важным способом отказа от этого значения по умолчанию.
...
Иногда небезопасный код имеет дело с указателями, которые могут висящими, быть смещены или указывать не на корректные данные. Чаще всего это встречается в  структурах с `repr(packed)`. В таком случае важно избегать создания ссылки, так как это может привести к неопределенному поведению. Это означает, что обычные операторы `&` и `&mut` не могут быть использованы, так как они создают ссылку. И слишком поздно избегать неопределённого поведения, даже если ссылка немедленно преобразуется в сырой указатель.
