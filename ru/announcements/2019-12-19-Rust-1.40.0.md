...
- [Генерация `macro_rules!` элементов в процедурных макросах.]
    ...
    Макросы подобные функциям (`mac!()`) и атрибуты (`#[mac]`) теперь могут генерировать элементы `macro_rules!`. Подробнее смотрите в прилагаемом отчёте о стабилизации.

- [Теперь сопоставление с помощью `$m:meta`] поддерживает [произвольные значения TokenStream].

    Теперь следующий код является корректным:
...
В релизе 1.35.0 [мы сообщили], что `NLL` появился в редакции Rust 2015 после первого выпуска для 2018 редакции в [Rust 1.31].
