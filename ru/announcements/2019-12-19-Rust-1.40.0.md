...
Предположим, вы являетесь автором крейт библиотеки `alpha`, которая содержит `pub struct Foo` . Вы хотели бы также сделать поля структуры `alpha::Foo` публичными `pub` , но не уверены, придется ли вам в будущих релизах добавить больше полей в `Foo`. Теперь возникает дилемма: либо вы делаете поля приватными с последующими за этим неудобствами, либо вы рискуете поставить пользователей в зависимость от этих полей и нарушить их код потом при добавлении новых полей. В Rust 1.40.0 представлен способ решить загвоздку с помощью : `#[non_exhaustive]` .

Атрибут `#[non_exhaustive]` , когда он прикреплен к структуре `struct` или варианту перечисления `enum` , будет препятствовать тому, чтобы код создавал бы упомянутую структуру `struct` или вариант вне крейта с их объявлением. Для избежания поломки в будущем, другие крейты также не будут выполнять полное сопоставление полей. Следующий пример иллюстрирует ошибки в крейте `beta`, который зависит от крейта `alpha` :
...
Возможно, более важным аспектом `#[non_exhaustive]` является то, что аттрибут также может быть прикреплен к самим перечислениям `enum` . Как пример, код взятый из стандартной библиотеки для [`Ordering`] :
...
Для более подробной информации об атрибуте `#[non_exhaustive]`, смотрите [отчет о стабилизации].
...
- [Генерация `macro_rules!` элементов в процедурных макросах.]
    ...
    Функционально похожие макросы (`mac!()`) и атрибуты (`#[mac]`) теперь могут генерировать элементы `macro_rules!` . Подробнее смотрите в прилагаемом отчете о стабилизации.

- [Теперь `$m:meta` матчер] поддерживает [произвольные значения потоковых токенов].
...
[отчет о стабилизации]: https://github.com/rust-lang/rust/issues/44109#issuecomment-533356866
...
[Теперь `$m:meta` матчер]: https://github.com/rust-lang/rust/pull/63674
